...
export async function GET(request: Request) {
  try {
    ...
    return Response.json({ authenticated: false, demoAvailable: isDemoAuthEnabled(), githubAvailable: Boolean(process.env.GITHUB_CLIENT_ID?.trim() && process.env.GITHUB_CLIENT_SECRET?.trim()) }, { headers: { "Cache-Control": "private, no-store" } });
  }
}
