...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json({ session: await getResearchAssistantSession(decodeURIComponent(id), authorized.auth.workspace.id) }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) {
