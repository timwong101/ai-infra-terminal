...
import { AlertTriangle, ArrowDownRight, ArrowUpRight, ExternalLink, FileText, LoaderCircle, Plus, Target } from "lucide-react";
...
export function ThesisWorkspace() {
  ...
  return <div className="research-workspace thesis-workspace">
    ...
    <section className="thesis-layout">
      ...
      <article className="panel thesis-detail-panel">{active ? <>{active.isStale && <div className="stale-research-banner"><AlertTriangle size={15} /><div><strong>Thesis score needs refresh</strong><span>{active.staleReason}</span></div></div>}<header><div><span className="section-kicker">{active.companyName} / {active.theme}</span><h2>{active.title}</h2><p>{active.statement}</p></div><div className={`thesis-big-score ${active.supportScore >= 50 ? "positive" : "negative"}`}><strong>{active.supportScore}</strong><span>support</span></div></header><div className="thesis-history"><div><span>Score history</span><small>{active.snapshots.length} evidence checkpoints</small></div><div className="thesis-bars">{active.snapshots.map((point, index) => <i key={`${point.date}-${index}`} style={{ height: `${Math.max(8, point.supportScore)}%` }} title={`${point.date}: ${point.supportScore}`} />)}</div></div><section className="thesis-evidence-list"><h3>Claim-to-evidence links</h3>{active.evidence.map((item) => <article key={item.id}><span className={`impact-pill ${item.impact}`}>{item.impact === "supports" ? <ArrowUpRight size={12} /> : item.impact === "weakens" ? <ArrowDownRight size={12} /> : <FileText size={12} />}{item.impact} {item.impactScore > 0 ? "+" : ""}{item.impactScore}</span><div><strong>{item.sourceType} · {item.documentTitle}</strong><p>{item.excerpt}</p><small>{item.documentDate} · {item.rationale}</small></div>{item.sourceUrl && <a href={item.sourceUrl} target="_blank" rel="noreferrer" aria-label="Open source"><ExternalLink size={14} /></a>}</article>)}{!active.evidence.length && <p className="missing-evidence">No analyst-approved evidence is linked yet. Review a suggested claim link in the evidence workspace, then run the research cycle.</p>}</section></> : <div className="workspace-state full"><Target size={28} /><strong>No claims available</strong></div>}</article>
    </section>
  </div>;
}
