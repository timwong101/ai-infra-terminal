...
import { CommitmentsLedger } from "@/app/components/commitments-ledger";
...
export function CompanyIntelligenceWorkspace({ initialCompanyId = "", onCompanyChange }: Props) {
  ...
  const [view, setView] = useState<"company" | "commitments" | "peers">("company");
  ...
  return <div className="research-workspace company-intelligence-workspace">
    <header className="workspace-title-row"><div><p className="breadcrumb">Research workspace / Temporal analysis</p><h1>Company Intelligence</h1><p className="workspace-subtitle">Reporting-period changes, management commitments, and comparable source-linked metrics across Neoclouds.</p></div><div className="company-view-actions"><div className="view-segmented-control" aria-label="Company intelligence view"><button aria-label="Company view" className={view === "company" ? "active" : ""} onClick={() => setView("company")}><Building2 size={14} /> Company</button><button aria-label="Commitments view" className={view === "commitments" ? "active" : ""} onClick={() => setView("commitments")}><ListChecks size={14} /> Commitments</button><button aria-label="Peer benchmark" className={view === "peers" ? "active" : ""} onClick={() => setView("peers")}><Rows3 size={14} /> Peers</button></div>{view === "company" && <button className="command-button" aria-label="Refresh company analysis" title="Refresh company analysis" disabled={status === "loading"} onClick={() => void load({ company: data?.company.id, sync: true })}>{status === "loading" ? <LoaderCircle className="drawer-spinner" size={15} /> : <RefreshCw size={15} />} Refresh analysis</button>}</div></header>
    {error && <div className="builder-error"><ShieldAlert size={14} /> {error}</div>}
    {view === "peers" ? <MetricBenchmark /> : view === "commitments" && data ? <CommitmentsLedger companyId={data.company.id} companies={data.companies} onCompanyChange={selectCompany} /> : data && <section className="company-intelligence-layout">
