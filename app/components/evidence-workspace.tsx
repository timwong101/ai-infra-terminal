...
export function EvidenceWorkspace({ initialCompanyId = "", onBuildComparison, onCompanyChange }: Props) {
      ...
      <div className="evidence-review-layout">
        <section className="evidence-catalog panel">
          ...
          {status === "ready" && <div className="evidence-catalog-list">
            {visibleItems.map((item) => <article className={`catalog-row ${selected?.id === item.id ? "selected" : ""}`} data-evidence-id={item.id} key={item.id} tabIndex={0} onClick={() => selectEvidence(item)} onKeyDown={(event) => { if (event.key === "Enter") selectEvidence(item); }}>
        ...
        <aside className="source-context panel">
          ...
          {selected ? <div className="source-context-body">
            ...
            <p className="provenance-note"><ShieldCheck size={14} /> {selected.reviewedBy ? `Reviewed by ${selected.reviewedBy.name}${selected.reviewedAt ? ` on ${new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric" }).format(new Date(selected.reviewedAt))}` : ""}.` : "Scores prioritize review; only an analyst decision makes a passage memo-eligible."}</p>
            <SourceProvenancePanel sourceKind={selected.sourceKind} sourceDocumentId={selected.sourceDocumentId} onPromoted={() => void load()} />
