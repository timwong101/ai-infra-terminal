...
import type { CopilotFilters, CopilotMessage, CopilotSession } from "@/lib/research/types";
...
const EMPTY_FILTERS: CopilotFilters = { companyIds: [], topic: "All topics", sourceKinds: [], dateFrom: undefined, dateTo: undefined };
...
export function CopilotWorkspace({ initialSessionId = "", onSessionSelect, onOpenMemo }: Props) {
  const creating = useRef(false);
  const filtersRef = useRef<CopilotFilters>(EMPTY_FILTERS);
  const [catalog, setCatalog] = useState<Catalog | null>(null);
  const [session, setSession] = useState<CopilotSession | null>(null);
  const [filters, setFilters] = useState<CopilotFilters>(EMPTY_FILTERS);
  const [question, setQuestion] = useState("");
  const [loadStatus, setLoadStatus] = useState<"loading" | "ready" | "error">("loading");
  const [notice, setNotice] = useState("");

  const loadCatalog = useCallback(async () => {
    const response = await fetch("/api/research-copilot", { cache: "no-store" });
  ...
  const loadSession = useCallback(async (id: string) => {
    const response = await fetch(`/api/research-copilot/${encodeURIComponent(id)}`, { cache: "no-store" });
    const result = await response.json() as { session: CopilotSession } | { error: string };
  ...
  const transport = useMemo(() => new DefaultChatTransport({ api: initialSessionId ? `/api/research-copilot/${encodeURIComponent(initialSessionId)}/messages` : "/api/research-copilot/invalid/messages" }), [initialSessionId]);
  ...
  const createSession = useCallback(async (preferredFilters?: CopilotFilters) => {
    if (creating.current) return;
    creating.current = true;
    try {
      const response = await fetch("/api/research-copilot", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ filters: preferredFilters ?? filtersRef.current }) });
  ...
  return (
    <div className="copilot-workspace">
      <aside className="copilot-history">
        <div className="copilot-history-heading"><div><span className="section-kicker">Saved work</span><h2>Question History</h2></div><button className="icon-button" onClick={() => void createSession()} title="New research question" aria-label="New research question"><Plus size={16} /></button></div>
        <div className="copilot-history-list">
          ...
          {!catalog?.sessions.length && <div className="copilot-history-empty">Your saved research questions will appear here.</div>}
        </div>
      </aside>
...
function SavedAnswer({ message, catalog }: { message: CopilotMessage; catalog: Catalog | null }) {
  return <article className="saved-answer">
    <div className="copilot-question"><span>Research question</span><p>{message.question}</p></div>
